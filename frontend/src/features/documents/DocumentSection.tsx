...
export default function DocumentSection({
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h2 className="font-semibold text-slate-900 dark:text-slate-100">Dokümanlar</h2>
  ...
  return (
    <Card className="flex flex-wrap items-end gap-4 p-4">
      ...
      <label className="flex flex-1 flex-col gap-1.5">
        <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Dosya</span>
        <input
          ref={inputRef}
          type="file"
          accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.csv,.txt,.png,.jpg,.jpeg"
          onChange={(e) => setFile(e.target.files?.[0] ?? null)}
          className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 file:mr-3 file:rounded-md file:border-0 file:bg-slate-100 file:px-3 file:py-1 file:text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:file:bg-slate-800 dark:file:text-slate-200"
        />
      </label>
      ...
      {upload.error ? (
        <p className="w-full text-sm text-rose-600">{upload.error.message}</p>
      ) : (
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
        <div className="min-w-56 flex-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <p className="font-medium break-all text-slate-900 dark:text-slate-100">
              {item.fileName}
            </p>
          </div>
