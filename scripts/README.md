...
| `render_vps.py` | Canlı VPS dağıtımının gerçek tarayıcıda doğrulaması: giriş ekranı ve KVKK onay kapısı, CSP'nin kendi paketini engellemediği, listenin/panonun veriyle çizilmesi, Karar Verici'nin denetim izine girememesi, sıfır konsol hatası. Adres `T3_VPS_BASE` ile verilir (11 kontrol) |
