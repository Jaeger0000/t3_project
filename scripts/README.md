...
| `render_vps.py` | Canlı VPS dağıtımının gerçek tarayıcıda doğrulaması: giriş ekranı ve KVKK onay kapısı, CSP'nin kendi paketini engellemediği, logonun çizilmesi, listenin/panonun veriyle çizilmesi, **logo paketi sayfası** (dört marka rengi, Pantone/CMYK, kendi sunucumuzdan gelen Barlow, renk kopyalama ipucu, üç zemin modu), Karar Verici'nin denetim izine girememesi, sıfır konsol hatası. Adres `T3_VPS_BASE` ile verilir (21 kontrol) |
